...
module RPG
  module Path
    FindFirstFile = Win32API.new("RTP.dll", "FFF", "P", "P")
    ...
    def self.findP(*paths)
      for p in paths
        findFileData = FindFirstFile.call(p.to_unicode)
...
module Audio
  class << self
    alias bgm_play_path bgm_play
    alias bgs_play_path bgs_play
    alias se_play_path se_play
    alias me_play_path me_play
  end
end

module Audio
  def self.bgm_play(filename, volume = 100, pitch = 1)
    path = filename
    path = RPG::Path::Graphics(path)
    bgm_play_path(path,volume,pitch)
  end
  def self.bgs_play(filename, volume = 100, pitch = 1)
    path = filename
    path = RPG::Path::Graphics(path)
    bgs_play_path(path,volume,pitch)
  end
  def self.me_play(filename, volume = 100, pitch = 1)
    path = filename
    path = RPG::Path::Graphics(path)
    me_play_path(path,volume,pitch)
  end
  def self.se_play(filename, volume = 100, pitch = 1)
    path = filename
    path = RPG::Path::Graphics(path)
